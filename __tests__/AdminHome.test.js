...
jest.mock('toastify-react-native', () => ({
  ...
  it('handles addMovie error gracefully', async () => {
    AdminApi.getMovies.mockResolvedValueOnce({ movies: [] });
    AdminApi.addMovie.mockRejectedValueOnce(new Error('Network error'));
    const { getByText } = render(<AdminHome />);
    await waitFor(() => expect(getByText('+ Add New Movie')).toBeTruthy());
    fireEvent.press(getByText('+ Add New Movie'));
    waitFor(() => {
      expect(require('toastify-react-native').Toast.error).toHaveBeenCalledWith('Error Failed to add movie');
    });
  });